  ...
  <section>
    ...
    <div class="search-box">
      <el-form size="mini" :inline="true" class="demo-form-inline">
        ...
        <el-form-item label="团队状态:">
          <el-select clearable v-model="params.teamStatus" placeholder="请选择组队状态">
            <el-option
              v-for="item in tdzts"
              :key="item.code"
              :label="item.name"
        ...
        <el-form-item label="团队名称: ">
          <el-input v-model="params.teamName" placeholder="请输入团队名称"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <div class="main-body">
      <el-table border :loading="loading" :data="data" stripe size="mini" style="width: 100%">
        ...
        <el-table-column  label="团队状态" width="100" >
          <div slot-scope="scope">
            {{scope.row.teamStatus==0?'尚未完成组队':scope.row.teamStatus==1&&scope.row.praxisStatus==null?'组队完成尚未创建项目':scope.row.teamStatus==1&&scope.row.praxisStatus!=null?'已创建项目':''}}
          </div>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" align="left">
          <div slot-scope="scope">
            ...
            <el-button type="text" class="opreat-btn"
                       v-if="scope.row.teamStatus==1&&scope.row.praxisStatus!=null"
                       @click.native=""
                      >查看项目
            </el-button>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      showDetail(id) {
        this.$router.push({ path: "/social/teamdetall", query: { teamid: id } });
      },
         clearzx(){
