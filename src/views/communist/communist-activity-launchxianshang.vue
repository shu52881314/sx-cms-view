  ...
  <section>
    ...
    <el-form :model="activity" :rules="rules" ref="activity" label-width="155px" class="demo-activity">
      ...
      <el-form-item label="活动主办方：" prop="orgId">
        <el-select  v-model="activity.orgId" placeholder="请选择活动主办方" style="width:30%" @change="getmod">
          <el-option
            v-for="(item,index) in organizationes"
            :key="index"
            :label="item.name"
            :value="item.id">
          </el-option>
...
" style="width:30%">
      ...
      <el-form-item label="是否申报经费物资：" prop="substanceLimit" v-if="wuzi==1?true:false">
        <el-radio-group v-model="activity.substanceLimit">
          <el-radio label="0">否</el-radio>
          <el-radio label="1">是</el-radio>
        </el-radio-group>
      </el-form-item>
      <!--<el-form-item label="经费物资：" prop="material" v-if="activity.substanceLimit==1?true:false">-->
        <!--<el-input v-model="activity.material" style="width:30%" placeholder="请输入经费物资" maxlength='30'></el-input>-->
      <!--</el-form-item>-->
      ...
      <!--<el-form-item label="活动场地：" prop="pitchId" v-if="changdi==1?true:false">-->
        <!--<el-select clearable v-model="activity.pitchId" placeholder="请选择场地" style="width:30%">-->
          <!--<el-option-->
            <!--v-for="(item,index) in pitchNames"-->
            <!--:key="index"-->
            <!--:label="item.name"-->
            <!--:value="item.id">-->
          <!--</el-option>-->
        <!--</el-select>-->
      <!--</el-form-item>-->
      <!--<el-form-item label="活动场地：" prop="address" v-if="changdi==1?false:true">-->
        <!--<el-input v-model="activity.address" style="width:30%" placeholder="请输入活动场地" maxlength='30'></el-input>-->
      <!--</el-form-item>-->
      <!--<el-form-item label="交作业：" prop="homeworkLimit">-->
        <!--<el-radio-group v-model="activity.homeworkLimit">-->
          <!--<el-radio label="0">否</el-radio>-->
          <!--<el-radio label="1">是</el-radio>-->
        <!--</el-radio-group>-->
      <!--</el-form-item>-->
      <!--<el-form-item label="是否必须提交：" prop="ishomeworksLimit" v-if="activity.homeworkLimit==1?true:false">-->
        <!--<el-radio-group v-model="activity.ishomeworksLimit">-->
          <!--<el-radio label="0">否</el-radio>-->
          <!--<el-radio label="1">是</el-radio>-->
        <!--</el-radio-group>-->
      <!--</el-form-item>-->
      ...
      <el-form-item label="活动限制人数：">
        <el-input type="number"  v-model.number="activity.peopleLimit" placeholder="不填表示不限制" style="width: 30%"></el-input>
      </el-form-item>

      <el-form-item label="参与院系：" prop="departmentId" >
        <el-radio-group  v-model="activity.departmentIdLimit2" @change="clears(1)">
      ...
      <el-form-item label="参与年级：">
        ...
        <el-radio-group v-model="activity.gradeLimits2" @change="clears(2)">
          <el-radio label="0">全部</el-radio>
          <el-radio label="1">选择</el-radio>
        </el-radio-group>
        <template>
          <div>
            <el-select style="width:30%" multiple v-model="activity.gradeIdzc" placeholder="请选择"
              ...
              <el-option v-for="(tag,index) in grades"  :key="index" :value="tag.id"
      ...
      <el-form-item label="参与年制：">
        ...
        <el-radio-group  v-model="activity.year2" @change="clears(3)">
          <el-radio label="0">全部</el-radio>
          <el-radio label="1">选择</el-radio>
        </el-radio-group>
      ...
      <el-form-item label="参与组织：">
        ...
        <el-radio-group  v-model="activity.tribeIdLimit2" @change="clears(4)">
          <el-radio label="0">全部</el-radio>
          <el-radio label="1">选择</el-radio>
        </el-radio-group>
      ...
      <el-form-item>
        <el-button type="primary" @click="submit('activity')" style="margin: 15px;">确认修改</el-button>
      </el-form-item>
    </el-form>
  </section>
</template>
  ...
  export default {
    ...
    methods: {
      getxiangqing() {
        ...
        this.$http.get('/activity/detailById', {
          ...
          if (res.code == 200) {
            ...
            this.activity.name=res.data.actName
            this.getClassify();
            // this.getSponsor();
            this.getxb();
            this.getlx();
            ...
            this.getOraganziton()
            this.getClass()
            this.getgly(res.data.adminId,res.data.adminName)
            this.getmod(res.data.orgId)
            if(res.data.orgId){
              let  tmp={
                id:res.data.orgId,
                name:res.data.orgName
              }
              this. organizationes.push(tmp)
            }
            // 回显
            this.activity.categoryId=res.data.classId
            this.activity.activitytypeId=res.data.type
            this.activity.orgId=res.data.orgId
            this.activity.id=res.data.actId

            this.activity.coorganizerId=res.data.xiebanId
            this.activity.img=res.data.logo
            this.activity.desc=res.data.introduce
            this.activity.pitchId=res.data.pitchId
            this.activity.address=res.data.pitchAddress
            this.activity.substanceLimit=res.data.expenditureAudit+""
            this.activity.material=res.data.expenditure
            this.activity.homeworkLimit=res.data.job+""
            this.activity.ishomeworksLimit=res.data.subJob+""


            for (let i=0;i<res.data.files.length;i++){
              res.data.files[i].progress=100;
              res.data.files[i].async = true;
            }
            this.activity.attachUrls2 =res.data.files
            this.activity.contact =res.data.adminContact
            this.activity.signLimit =res.data.signAudit+""
            this.activity.peopleLimit =res.data.peopleLimit
            if(res.data.gender==null){
              this.activity.sexId ="0"
            }else {
              this.activity.sexId =res.data.gender
            }

            this.activity.bms = new Date(res.data.enrollEndTime)
            this.activity.activityDate = [new Date(res.data.starTime), new Date(res.data.endTime)];

            if(res.data.collegeIds==null){
              this.activity.departmentIdLimit2='0'
            }else {
              this.activity.departmentIdLimit2='1'
              let  arr=res.data.collegeIds.split(",")
              let arr2=[]
              for (let i=0;i<arr.length;i++){
                arr2.push(parseInt(arr[i]))
              }
              this.activity.departmentIdzc=arr2
            }

            if(res.data.gradeIds==null){
              this.activity.gradeLimits2='0'
            }else {
              this.activity.gradeLimits2='1'
              let  arr=res.data.gradeIds.split(",")
              let arr2=[]
              for (let i=0;i<arr.length;i++){
                arr2.push(parseInt(arr[i]))
              }
              this.activity.gradeIdzc=arr2
            }
            if(res.data.yearIds==null){
              this.activity.year2='0'
            }else {
              this.activity.year2='1'
              let  arr=res.data.yearIds.split(",")
              let arr2=[]
              for (let i=0;i<arr.length;i++){
                arr2.push(parseInt(arr[i]))
              }
              this.activity.yearzc=arr2
            }
            if(res.data.zuzhiIds==null){
              this.activity.tribeIdLimit2='0'
            }else {
              this.activity.tribeIdLimit2='1'
              let  arr=res.data.zuzhiIds.split(",")
              let arr2=[]
              for (let i=0;i<arr.length;i++){
                arr2.push(parseInt(arr[i]))
              }
              this.activity.cyorgIdzc=arr2
            }

            this.activity.foulLimit =res.data.foulLimit
      ...
      getActivitTplByParty(val) {
        // 获取模板
        this.$http.get("/school/getActivitTplByParty",{params:{ orgId: val}} ).then(res => {
          if (res.data) {
      ...
      submit(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            that.$http.post("/activity/edit-activity", that.activity).then(res => {
              if (res.code == "200") {
                ...
                that.$router.push({path:"/communist/activity", query: {oto:1}})
              }
            });
