<template>
  <div>
    ...
    <div class="main-body content" style="margin: 10px;border: 1px #e4e4e4 solid;color: #000000;padding-bottom: 25px;">
      ...
      <div class="" style="border-bottom:1px solid #e4e4e4">
        ...
        <div class="body-part1">
          ...
          <div class="">
            <span>申请人姓名：{{data.userName}}</span><br/>

            <span>所在院系：{{data.collegeName}}</span>
          </div>
          <div class="">
            <span>学号：{{data.code}}</span><br/>
            <span>专业：{{data.majorName}}</span>
          </div>
          <div class="">
            <span>年级：{{data.grade}}</span><br/>
            <span>班级：{{data.className}}</span>
          </div>
      ...
      <div class="body-part3">
        ...
        <el-form size="mini" label-position="left">
          ...
          <el-form-item label="项目时间：">
            {{project.startTime | formatD}} 至 {{project.endTime | formatD}}
          </el-form-item>
        ...
        <div v-if="project.files && project.files.length">
          <span style="display: block;margin-bottom:1rem;font-size: 14px;color: #606266;">证明材料：</span>
          <div class="files">
            <div
              style="padding:.6rem;width:140px;display: flex;flex-direction: column;align-items: center;"
              ...
              <div style="text-align: center">
                <span class="showOverflow" :title="item.name">{{item.name.length >= 8 ? item.name.substring(0,8) + '...' : item.name}}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="main-body content" style="margin: 10px;border: 1px #e4e4e4 solid;color: #000000;padding-bottom: 25px;">
      <div class='body-header'>
        当前状态：{{project.status == 2 ? '待审核' : (project.status == 1 ? '通过' : project.status == 0 ? '驳回' : '未知状态')}}
      </div>
      ...
      <div v-if="show">
        <el-table border :data="auditLog" stripe size="mini" style="width: 100%">
          ...
          <el-table-column  label="备注信息" align="center">
            <template slot-scope="scope">
              {{scope.row.auditIdea === 1 ? '分值：'+ scope.row.credit+hoursUnit: scope.row.auditIdea === 0 ? scope.row.remark :  scope.row.remark}}
            </template>
          </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initData() {
        this.$http.get("/school/student/info", {params: {userId: this.$route.query.id}}).then((res) => {
          this.data = res.data;
        })
        this.$http.get("/project/record/details", {
          params: {
          ...
          for (let i = 0; i < this.project.files.length; i++) {
            ...
            let type = temp.url.substring(temp.url.lastIndexOf('.') + 1).toLocaleLowerCase();
            if (type.indexOf('jpeg') != -1 || type.indexOf('jpg') != -1 || type.indexOf('jfif') != -1 || type.indexOf('bmp') != -1 || type.indexOf('gif') != -1 || type.indexOf('png') != -1 || type.indexOf('tiff') != -1) {
              ...
              temp.type = 3;
            } else if (type.indexOf('xlsx') != -1 || type.indexOf('xls') != -1) {
              temp.type = 4;
          ...
          this.$http.get("/project/audit/log", {params: {applyId: this.$route.query.applyId}}).then((re) => {
