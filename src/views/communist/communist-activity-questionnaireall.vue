...
<template>
  <div>
    ...
    padding: 10px 0;
   ">
            <div class="title" style=" float: left;margin-left: 15px;">问卷名称:</div>
    ...
    padding: 10px 0;
    ">
            <div class="title" style=" float: left;margin-left: 15px;">问卷说明:</div>
            <div style="width: 65%;margin-left: 10%; line-height: 24px;">{{item.introduction}}</div>
          </div>
          <div v-for=" (ti,index) in item.questions" style="  width: 75%;margin-top: 10px;">
            <div v-if="ti.type==1">
              <p class="xinxitou"><span style="margin-right: 5px">{{index+1}}.</span><span>{{ti.questionStem}}</span></p>
              <p v-for=" (xuanxiang,i) in ti.options" style="padding: 10px" class="xuan">
...
<script>
  ...
  export default {
    ...
    methods:{
      chakanwj(applyId){
        this.$router.push({path:"/proceeding/wait/activity/questionnaire", query: {applyId:applyId,all:1}})
      },
      dolg(asw){
        if(asw.length<=0){
          this.$message({
