  ...
  <section>
    ...
    <el-dialog title="批量"
     ...
     width='650px' id="dialogOrange">
         ...
         <div v-show="!showfileInfor">
           ...
           <div class="uploadLocal" v-show="fileList.length">
             ...
             <div class="uploadLocal-btn">
              ...
              <button class="uploadLocal-btnSubmit"  @click="submitUpload">确认</button>
             </div>
           </div>
            ...
            <el-form v-show="!fileList.length" size='mini' label-width='12px'>
                <div class="importTipsTitle">
                  ...
                  <span class="importTipsTitle-right">
                    <a class="el-button el-button--text el-button--mini" download="" href='/static/tpls/减分项模板.xlsx' size='mini' >模板下载</a>
                  </span>
                </div>
       ...
       <back></back>
	    <el-dialog
    		title="添加减分项学生" @close="beforeCloseHandler"
		...
		<el-form  :model="ruleForm" :rules="rules"  ref="ruleForm" label-width="110px" class="demo-ruleForm">
			...
			<el-form-item :label=" '减' + hoursUnit" prop="hours">
				<el-col :span="22" >
		...
		<div class="main-header">
			<span class="main-title">
				<p style="width: 60%;">减分项详情</p>
				<span style="font-size:12px;color:#999">学生有不当行为需要减分的，可进行添加或批量导入</span>
			</span>
          ...
          <el-button   type="primary" size="mini" @click="dialogImportCollegeVisible = true" >导入学生</el-button>
					<el-button @click="dialogVisible = true" type="primary" size="mini" v-perms="'/communist/sub/student/add'">添加学生</el-button>
			</div>
		</div>

		<div class="plusShow-towTitle">
			<p style="font-size:16px;color:#333">{{showDetail.name}}</p>
			<div style="overflow: hidden;"><div style="width: 20%;float: left;">分类名称：{{showDetail.classifyName}}</div><div style="width: 20%;float: left;">限定类型：{{showDetail.limitType | formatLimitType}}</div><div style="width: 20%;float: left;">添加时间：{{showDetail.ctime | formatDate}}</div><div style="width: 20%;float: left;">添加人：{{showDetail.creatorName}}</div></div>
		...
		<el-table :loading="loading" :data="data" stripe size="mini" style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="hours" :label="'减' + hoursUnit"  show-overflow-tooltip></el-table-column>
  ...
  data() {
    ...
    downFailed(){
      ...
      axios({
       ...
       link.href = url
       link.setAttribute('download', '减分项成员失败信息' + '.xlsx')
