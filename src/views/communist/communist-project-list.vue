        ...
        <el-table-column label="操作" width="200" align="left">
          <div slot-scope="scope">
            ...
            <el-button v-if="scope.row.status == 1" type="text" class="opreat-btn"
                       @click.native="stopProject(scope.row.projectId,2)" 
                       v-perms="'/communist/project/stop'">停止申请
            </el-button>
            <el-button v-if="scope.row.status == 2" type="text" class="opreat-btn"
                       @click.native="stopProject(scope.row.projectId,1)" 
                       v-perms="'/communist/project/stop'">恢复申请
            </el-button>
            <el-button type="text" class="opreat-btn" 
                       @click.native="showDetail(scope.row.projectId)"
                       v-perms="'/communist/project/details'">详情
            </el-button>
            <!-- <el-button v-if="scope.row.applyPassNumber == 0" type="text" class="opreat-btn"
                        @click.native="deleteProject(scope.row.projectId)" 
                        v-perms="'/communist/project/delete'">删除
             </el-button>-->
          </div>
        </el-table-column>
      </el-table>
