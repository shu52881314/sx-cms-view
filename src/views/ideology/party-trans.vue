    ...
    <el-dialog title="申请信息" :visible.sync="dialogVisiblesee" :close-on-click-modal="false" width="750px">
        ...
        <el-form size="mini" :model="ruleFormReject" ref="ruleFormReject" class="demo-ruleForm">
            <el-form-item prop="name">
                <el-col :span="22">
                    <el-input type="textarea" placeholder="请输入驳回理由" v-model="ruleFormReject.remark" maxlength="200"></el-input>
                </el-col>
            </el-form-item>
            <el-form-item>
		...
		<div class="main-header">
			<span class="main-title">党员转接</span>
		</div>
		...
		<div class="main-body">
          ...
          <el-table-column prop="userIdentity" label="身份">
            <div slot-scope="scope">
              {{scope.row.userIdentity == 3 ? "入党积极分子" : (scope.row.userIdentity == 4 ? "发展对象" : (scope.row.userIdentity == 5 ? "预备党员" : "党员" ) ) }}
            </div>
          </el-table-column>
          <el-table-column prop="organizationName" label="所属党组织" ></el-table-column>
          <el-table-column prop="transferInfo" label="转移至" ></el-table-column>
          ...
          <el-table-column  label="操作" align="center">
            <div slot-scope="scope">
              <!-- 可转载站点功能未完成 -->
              <span v-perms="'/ideology/party/member/transfer/examine'">
              ...
              <el-button  v-if="scope.row.status == 2" type="text" class="opreat-btn">已通过</el-button>
    ...
    openMore(item){
      ...
      this.attachShow = item
		...
		submitFormPass() {
          ...
          this.$http.post('ideology/transfer/audio',this.ruleFormPass).then( () => {
		  ...
		  submitFormReject() {
            if (this.ruleFormReject.remark) {
                this.$http.get('/ideology/transfer/audio',{params:this.ruleFormReject}).then( (res) => {
                	if(res.code == '200'){
		...
		fetch() {
			this.loading = true
			this.$http.get('/ideology/transform/list', {params: this.params}).then(res => {
      ...
      let that = this
      if (status == 2) {
        that.ruleFormPass.id = row.id;
        that.$confirm('是否通过该审核?', '提示', {
  								confirmButtonText: '确定',
  								cancelButtonText: '取消',
  								type: 'warning'
  						}).then(() => {
  				that.$http.get('ideology/transfer/audio',{params:this.ruleFormPass}).then(res => {
  					if(res.code == '200'){
                 	that.$message({
										type: 'success',
										message: '通过成功!'
