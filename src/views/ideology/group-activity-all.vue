		...
		<div class="main-body">
          ...
          <el-table-column  label="操作" width="160" align="left">
            <div slot-scope="scope">
              <!-- 可转载站点功能未完成 -->
              <el-button type="text" class="opreat-btn" @click.native="routerLink('/communist/activity/detail','view',scope.row.id)" v-perms="'/communist/activity/detail'">详情</el-button>
              <el-button type="text" class="opreat-btn"  v-if="scope.row.status < 8" @click.native="routerLink('activity/found', 'update', scope.row.id)" v-perms="'/communist/activity/modify'">修改</el-button>
              <el-button type="text" class="opreat-btn"  v-if="scope.row.status < 7"  @click.native="deleteBatch(scope.row.id,scope.row.status,scope.row.name) " v-perms="'/communist/activity/delete'">删除</el-button>
            </div>
          </el-table-column>
        </el-table>
  ...
  data() {
    ...
    importFile(){
      ...
      axios({
       ...
       link.href = url
       link.setAttribute('download', '团建活动' + '.xlsx')
